...
def get_personal_info(user_url):
    ...
    try:
        db['user_info_2'].insert_one(person_dic)
    except Exception:
        print "insert error"
...
def get_users_info(like_users_url, post_id, page_id, update_time):
    ...
    for user_li in users_li[:-1]:
        ...
        try:
            # assert type((user_url)) == tuple, 'not tuple'
            thread.start_new_thread(get_personal_info, tuple([user_url]))
        except:
...
def fans_page_crawler(fans_page_url, page_id, update_time):
    ...
    flag = 0
    while current_url is not None and flag < 20:
        try:
...
if __name__ == "__main__":
    ...
    for i in range(len(fans_pages_url_list)):
        fans_page_crawler(fans_pages_url_list[i], pages_id_list[i], operate_time)
